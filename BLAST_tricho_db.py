...
def trim_and_db(entrada):
    ...
    for dato in datos:
        if dato[2] == False:
            ...
            comando = 'makeblastdb -in {0}.fna -title db_{1} -dbtype nucl -out db_{1}/db_{1} -parse_seqids'.format(dato[0].replace(' ','_'),dato[1])
...
filenames = glob.glob('*.fna')#+(glob.glob('*.gbff'))
paths = glob.glob('*/')
for organism in organisms:
    # internal search genome files by org name, record results
    if organism[0].replace(' ','_')+'.fna' in filenames :#or organism[0].replace(' ','_')+'.gbff' in filenames :
