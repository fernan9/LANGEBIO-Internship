...
"""
-----------------------------------------
BLAST XML filtering script May 20th, 2015
-----------------------------------------
-CONTACT-
Fernan Rodrigo Pérez Gálvez: fernan954<at>gmail<dot>com
-DESCRIPTION-
Gets inside each BLAST.XML file in working directory and filters
some information to retrieve an easy-to-load-in-R text file.
"""
import sys,csv
...
def carga_xml():
    ...
    lista = list()
    lista = glob.glob('*.xml')
    print 'Files with *.XML extention\n--------------------------'
    for i in lista:
        print (str(i)+'\n')
    return lista
...
# salutation
print('---------------\nBLAST FILTERING\n---------------')
print('Looking for XML files in working folder...')
# look for XML file in working directory
archivosXML = carga_xml()
raw_input('Press ENTER to continue...')
# go through files
for archivo in archivosXML:
    result_handle = open(str(archivo)+'.xml')
    ...
    for blast_record in blast_records:
        for alignment in blast_record.alignments:
            for hsp in alignment.hsps:
                ...
                pairing_size = hsp.align_length
                e_value = hsp.expect
                query_ID = blast_record.query
                sequence_ID = alignment.accession
    

# look for all *.xml in folder
# ask for corroboration
# extract query ID, bit score, Evalue, pairing size, paired sequence, position of beggining, sequence ID
